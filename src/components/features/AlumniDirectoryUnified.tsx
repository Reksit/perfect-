  ...
  const loadAlumniDirectory = async () => {
    try {
      ...
      try {
        ...
        response = await axios.get('https://finalbackendd.onrender.com/api/users/alumni', {
          headers: {
      ...
      const enhancedAlumni = await Promise.all(
        alumniData.map(async (alum: any) => {
          try {
            ...
            const profileResponse = await axios.get(`https://finalbackendd.onrender.com/api/alumni-profiles/complete-profile/${alum.id}`, {
            ...
            return {
              ...
              location: profileData.location || alum.location || 'Not specified', // Prioritize location from alumni_profiles
