...
const UserChat: React.FC = React.memo(() => {
  ...
  const loadAllUsers = async () => {
    try {
      ...
      console.log('UserChat: User roles breakdown:', response.reduce((acc: any, user: ChatUser) => {
  ...
  const filterUsers = () => {
    ...
    console.log('UserChat: Filtering users. Total before filter:', filtered.length);
    ...
    if (selectedFilter !== 'all') {
      ...
      console.log(`UserChat: After role filter (${selectedFilter}): ${filtered.length} (was ${beforeRoleFilter})`);
    }
    ...
    if (searchQuery.trim()) {
      ...
      console.log(`UserChat: After search filter (${searchQuery}): ${filtered.length} (was ${beforeSearchFilter})`);
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">Connect with students, alumni, and faculty</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-[500px] max-w-full">
        {/* Conversations Sidebar */}
        <div className="lg:col-span-1 bg-white/90 backdrop-blur-xl rounded-xl shadow-lg border border-white/20 flex flex-col">
            ...
            <div className="space-y-3">
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white px-4 py-2 rounded-xl shadow-lg shadow-blue-500/25 hover:shadow-xl hover:shadow-blue-500/30 transition-all duration-300 flex items-center justify-center space-x-2 font-medium"
                >
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white backdrop-blur-sm border border-gray-200 rounded-xl shadow-lg z-50 max-h-80 overflow-hidden">
                    ...
                    <div className="p-3 border-b border-gray-200 space-y-2">
                      <input
                        ...
                        placeholder={isSearching ? "Searching..." : "Search by name, email, or phone..."}
        ...
        <div className="lg:col-span-2">
          {selectedUser ? (
            <div className="bg-white/90 backdrop-blur-xl rounded-xl shadow-lg border border-white/20 flex flex-col h-full">
              {/* Chat Header */}
              <div className="p-3 border-b border-gray-200/50 flex items-center space-x-3">
                <div className="w-10 h-10 bg-gray-600 rounded-full flex items-center justify-center">
                  <User className="h-6 w-6 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-white truncate">{selectedUser.name}</h3>
                  <p className="text-sm text-gray-600 truncate">{selectedUser.email}</p>
                  <p className={`text-xs font-medium truncate ${getRoleColor(selectedUser.role)}`}>
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto p-3 space-y-3" style={{maxHeight: 'calc(500px - 120px)'}}>
                {messages.length === 0 ? (
              ...
              <div className="p-4 border-t border-gray-200/50">
                <form onSubmit={sendMessage} className="flex space-x-2">
                  <input
                    ...
                    className="flex-1 p-3 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    disabled={loading}
                  />
                  <button
                    type="submit"
            ...
            <div className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 p-8 text-center h-full flex items-center justify-center">
              <div>
